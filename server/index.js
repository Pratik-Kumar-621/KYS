...
app.post('/getBSE',async(req,res)=>{
    ...
    const findResult = client.db(DATBASE_NAME).collection('BSE').find({
        Date: {
          $gte: startDate,
          $lt: endDate,
        },
...
app.post('/getNSE',async (req,res)=>{
    let startDate = new Date(req.body.data.startDate)
    let endDate = new Date(req.body.data.endDate)
    const findResult = client.db(DATBASE_NAME).collection('NSE').find({
        Date: {
          $gte: startDate,
          $lt: endDate,
        },
...
app.post('/getCompany',async(req,res)=>{
    ...
    const findResult = client.db(DATBASE_NAME).collection(companyName).find({
        Date: {
          $gte: startDate,
          $lt: endDate,
        },
